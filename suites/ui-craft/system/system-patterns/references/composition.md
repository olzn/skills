...
Use for prompts about quality, craft, papercuts, entropy, or "why does this feel bad?" Read `quality.md` first, then route to the owning skills.

1. **system-patterns** (lead): workflow coherence, task completion, complexity cost, missing states
2. **system-components**: state models, component contracts, API consistency, downstream escape hatches
3. **surface-details**: papercuts, platform friction, focus, touch, scroll, performance, unsafe hit areas
4. **surface-copy**: errors, empty states, helper text, loading text, recovery language
5. **surface-interaction/surface-motion**: spatial logic, interruptibility, transition continuity, animation restraint
6. **surface-colour/surface-typography**: contrast, hierarchy, rhythm, visual balance
...
| Quality / craft / papercut review | See `quality.md` | system-patterns, system-components, surface-details |
...
Quality prompts are still routed, not handled as a separate domain. Use `quality.md` to classify the failure signal, then apply the smallest set of owning skills.
