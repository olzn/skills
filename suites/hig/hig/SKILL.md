...
1. Open `references/index.md` (≤1.5K tokens) → find the slug's file and anchor.
2. `grep -n "^## <slug>" references/corpus/<file>.md` → `Read` that file with `offset` at the match, `limit` ≈ section length (sections are 5–80 lines). Do not read whole corpus files for one question.
